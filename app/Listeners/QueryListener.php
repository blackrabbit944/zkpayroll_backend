...
class QueryListener
{
    public function handle(QueryExecuted $event)
    {
        if ( app()->environment(['local', 'testing'])) {
            ...
            $log = sprintf("(%sms) %s", $event->time, $log);
            $this->put_log('sql', $log);
            // Log::debug("query: {$sql} time: {$event->time}ms"); // 将SQL和执行时间打印到日志
        }
    }
    ...
    private function put_log($file = 'app', $content = '')
    {
        ...
        $content = sprintf('[%s.%03d]%s', date('Y-m-d H:i:s'), gettimeofday()['usec'] / 1000, $content);
